...
import {
  ...
  resolveJoinResolutionAfterConfirmation,
} from "../lib/sharedLedgerJoinResolution";
import {
...
export function SharedLedgerPanel({
  ...
  const handleTransferOwnership = async (targetUserId: string) => {
    ...
    const didTransfer = await onTransferOwnership(targetUserId);
    showNativeToast(
      didTransfer
        ? AppMessages.accountTransferOwnershipSuccess
        : AppMessages.accountTransferOwnershipError,
    );
  ...
  async function resolveConfirmedJoinResolution(
    ...
    const confirmationKind = resolveJoinPreviewConfirmationKind(joinPreview.status);

    if (confirmationKind === "standard") {
      ...
      return resolveJoinResolutionAfterConfirmation(joinPreview.status, didConfirm);
    }

    if (confirmationKind === "discard") {
      const didConfirm = await confirmJoinRequest(
    ...
    const resolution = resolveJoinResolutionAfterConfirmation(joinPreview.status, true);
    if (resolution) {
      return resolution;
    }
