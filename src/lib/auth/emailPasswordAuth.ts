...
function isEmailNotConfirmedSignInError(error: unknown): boolean {
...
export function resolveEmailPasswordSignInErrorMessage(error: unknown): string {
  if (isEmailNotConfirmedSignInError(error)) {
    return "이메일 인증이 아직 완료되지 않았어요. 가입 메일의 인증을 먼저 완료해 주세요.";
  }
...
export async function resendEmailSignUpOtp(email: string, captchaToken: string): Promise<void> {
