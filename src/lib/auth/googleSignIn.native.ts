...
let pendingGoogleRedirectUrl: string | null = null;
...
export function isGoogleSignInCancelled(error: unknown): boolean {
  ...
  return errorWithMessage.message === "Google 로그인이 취소되었어요.";
}
...
export async function completeGoogleSignInRedirect(redirectUrl: string): Promise<void> {
  ...
  if (pendingGoogleRedirectUrl === redirectUrl) {
    return;
  }

  pendingGoogleRedirectUrl = redirectUrl;
  try {
...
export async function signInWithGoogle(): Promise<void> {
  ...
  const authSessionResult = await openAuthSessionAsync(data.url, redirectUri);
  if (authSessionResult.type !== "success") {
    throw new Error("Google 로그인이 취소되었어요.");
  }

  await completeGoogleSignInRedirect(authSessionResult.url);
}
