...
type LedgerEntryNotificationRow = {
...
const DefaultProfilePushPreferences: ProfilePushPreferences = {
  currency: "KRW",
};
...
async function buildPushMessages(
  ...
  const notificationEvent = await resolveNotificationEvent(adminClient, payload);
  ...
  return resolveExpoPushMessages(adminClient, filteredUserIds, (preferences) =>
    buildExpoPushContent(notificationEvent, payload.widgetData, preferences, payload.bookId),
  );
}

async function resolveNotificationEvent(
  adminClient: SendPushNotificationsAdminClient,
  payload: Exclude<PushNotificationRequest, LatestJoinRequestOwnerPushRequest>,
): Promise<NotificationEvent> {
  if (payload.event.type !== "other_member_created_entry" || !payload.event.entryId) {
    return payload.event;
  }

  const bookId = "bookId" in payload ? payload.bookId : undefined;
  if (!bookId) {
    return payload.event;
  }

  const entryRow = await fetchLedgerEntryNotificationRow(
    adminClient,
    bookId,
  ...
  return {
    ...
    category: readText(entryRow.category ?? undefined, payload.event.category ?? ""),
    content: readText(entryRow.content ?? undefined, payload.event.content ?? ""),
    date: readText(entryRow.occurred_on, payload.event.date ?? ""),
    entryType: entryRow.entry_type,
    note: readText(entryRow.note ?? undefined, payload.event.note ?? ""),
  };
}

async function fetchLedgerEntryNotificationRow(
  adminClient: SendPushNotificationsAdminClient,
  bookId: string,
  entryId: string,
): Promise<LedgerEntryNotificationRow | null> {
  const entriesQuery = adminClient.from("ledger_entries") as {
    select: (columns: string) => {
      eq: (
        ...
        eq: (
          ...
          maybeSingle: () => QueryResult<LedgerEntryNotificationRow>;
        };
      };
    };
  };
  const { data, error } = await entriesQuery
    .select("id, book_id, entry_type, occurred_on, amount, content, category, note")
    .eq("book_id", bookId)
    .eq("id", entryId)
    .maybeSingle();
...
function buildExpoPushContent(
  event: NotificationEvent,
  widgetData?: WidgetPushData,
  preferences: ProfilePushPreferences = DefaultProfilePushPreferences,
  bookId?: string,
): ExpoPushContent {
  ...
  const data = {
    ...
    ...buildNotificationBadgeData(event, bookId ?? widgetData?.bookId),
  };
...
function buildNotificationBadgeData(
  event: NotificationEvent,
  fallbackBookId?: string,
): Record<string, string> {
  if (event.type !== "other_member_created_entry" || !event.entryId) {
    return {};
  }
...
function buildNotificationContent(
  ...
  const content = readText(event.content, translate(locale, "push.fallbacks.content"));
  const entryTypeLabel = event.entryType
    ? translate(locale, `push.entryTypes.${event.entryType}`)
    : translate(locale, "push.fallbacks.entry");
  const amountLabel =
