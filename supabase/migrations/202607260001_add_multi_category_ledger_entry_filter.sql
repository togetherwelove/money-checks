...
declare
  normalized_category_filters text[] := (
    select coalesce(
      array_agg(distinct trim(category_id)),
      array[null::text]
    )
...
revoke all on function public.get_enriched_ledger_entries_v2(
  uuid,
  date,
  date,
  text[],
...
grant execute on function public.get_enriched_ledger_entries_v2(
  uuid,
  date,
  date,
  text[],
